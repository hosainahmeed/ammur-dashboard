...
function CreateNewArchive({ archiveId, id, setCreateNewModal }) {
  ...
  const isLoading = createLoading || updateLoading || singleArchiveLoading;
  const isEditMode = !!id;
  ...
  const onFinish = async (values) => {
    ...
    try {
      const data = {
        ...
        archieveCategoryId: archiveId || id,
      };
      ...
      if (isEditMode) {
        await updateSubArchive({ id: archiveId, data: formData })
          .unwrap()
          .then((res) => {
            if (res?.success) {
      ...
      const errorMessage =
        ...
        error?.data?.message ||
        `Failed to ${isEditMode ? 'update' : 'create'} archive`;
  ...
  return (
    <Card bordered={true}>
      <Spin
        spinning={isLoading}
        tip={isEditMode ? 'Updating archive...' : 'Creating archive...'}
      >
